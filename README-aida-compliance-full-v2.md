...
**Archive:** the complete `aida-compliance` GitHub Copilot plugin — 7 skills, 1 agent,
1 prompt, always-on instructions, 670 tests and a one-command demo. The skill-only
submissions come from it; this one is packaged whole so it can be run end to end.
...
Every row points at the code that produces the answer and the test that proves it; all
of them are in this archive (`skills/…/scripts/`, `tests/`).
...
**Run it now, without installing anything** — from the archive's folder:
...
with 21 evaluation cases, two dated runs and two tagged releases — and runs every skill on
it: the filled document is `aida-demo/ticket_agent/aida/technical_documentation.docx`, the
36-requirement scorecard `aida/audit_report.md`, the record journal and oversight controls
`aida/generated/`, the two runnable monitors `aida/monitoring/`.

**See the result first, without installing anything** — open
`examples/ticket_agent/technical_documentation.docx` in the archive: the official template
...
The archive is the whole plugin; the technical documentation skill is marked.
...
├── run_all.py                   whole chain on a project in one command; --demo: the example
...
The archive holds **670 tests** — unit, integration, conformance, security posture, the
demo and the real-project lessons above — in `tests/` (`python -m pytest tests/ -q`).
`make_submission.py` extracts this zip and runs them inside it before declaring it ready.
