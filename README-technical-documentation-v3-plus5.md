...
| 2.2 / 2.3 Dependencies, compatibility, updates | code, with versions and `path:line`; CycloneDX ML-BOM | `build_values.r_dependencies`, `r_cicd`, `aida_bom.build_bom` | `test_dependencies_are_read_with_versions` |
...
| 4 Updates and changes | last change from git; cited files hashed, CI fails on drift | `build_values.r_substantial_changes`, `check_freshness.compare` | `test_a_code_change_marks_the_dependent_fields_stale` |
...
in the archive: the official template filled on a small LangGraph ticket agent, released
...
The repository holds **604 tests** — unit, integration, conformance, security posture,
...
|---|---|---|
| AST01 malicious behaviour, exfiltration | no network module in any script; writes only under `aida/`; the audited source is never modified | `test_no_script_imports_a_network_module`, `test_project_source_is_never_touched` |
...
| AST03 excessive permissions | `allowed-tools` limited to `shell(python:*)` per skill, `shell(git:*)` only for the skills that read history — never the whole shell | `test_allowed_tools_is_scoped_to_named_commands`, `test_git_is_only_granted_to_skills_that_read_git` |
