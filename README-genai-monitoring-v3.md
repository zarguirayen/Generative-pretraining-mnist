...
relies on `aida-core` for the scan and on `aida-record-keeping` for the journal it reads and
writes. Python standard library only; nothing to install.
...
| **Threshold** | `baseline × (1 − drop)` from the family's rule; latency and tokens alert on a rise; a looser threshold is blocked by the validator | `plan_monitoring.build_plan` | `test_threshold_is_derived_from_the_measured_score`, `test_latency_degrades_upwards_even_without_a_baseline` |
...
| **Window** | one period past the drop alerts, as the rule says; consecutive breaches configurable and tracked | `genai_monitor.consecutive` | `test_consecutive_breaches_are_honoured` |
...
| **Ground truth** | user feedback (`up` / `down` with its cause) and corrections joined to each inference by `request_id` | `genai_monitor.run` | `test_ground_truth_and_feedback_are_joined_by_request_id` |
...
| Section 3.8 | written from the plan, naming the monitor that runs | `write_section.build` | `test_section_3_8_describes_what_runs_not_what_is_intended` |
...
`examples/ticket_agent/technical_documentation.docx` in the archive: section 3.8 and the
...
│   ├── aida-record-keeping/          the journal the monitor reads and writes
│   ├── aida-operational-monitoring/  the operational half of 3.8, same journal
│   └── aida-core/                    scan, 36 rules, validator, Word writer
├── examples/ticket_agent/            a generated AIDA document to open directly
└── tests/  test_genai_monitor.py  test_monitoring.py  test_runtime_integration.py …   653 tests
```
...
- **Context footprint:** the seven skill descriptions (~1,200 tokens) are always loaded; this
  task activates one `SKILL.md` (~2,400 tokens). Scripts are executed, not read.
...
| AST03 permissions | `allowed-tools` limited to `shell(python:*)`, plus `shell(git:*)` for tool-set drift | `test_allowed_tools_is_scoped_to_named_commands`, `test_git_is_only_granted_to_skills_that_read_git` |
