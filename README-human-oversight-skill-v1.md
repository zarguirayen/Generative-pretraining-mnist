...
**Archive:** the skill folder `aida-human-oversight/`, complete — `SKILL.md`, three scripts,
the controls template, the requirements reference. It is one of the seven skills of the
`aida-compliance` GitHub Copilot plugin and runs inside it: it relies on `aida-core` and
writes its interventions into the record-keeping journal — declared in `depends_on`,
listed file by file in `SKILL.md` › Prerequisites. Python standard library only; nothing
to install.
...
Scripts named here are in the archive. The tests are in the plugin's `tests/` folder: 658 tests, all passing on this exact skill folder. `make_submission.py --skill` re-runs them with the archived copy in place before declaring the zip ready.
...
plugin: section 3.9 is written by this skill, and the architecture diagram draws the two
tools without an approval gate in red.

## Repo structure

```
aida-human-oversight/                       ← this archive
...
Word writer), `aida-record-keeping/` (the chained journal and its verifier), `examples/`
and `tests/` (`test_human_oversight.py`, `test_agentic_coverage.py`,
...
- **Context footprint:** the seven skill descriptions (~1,200 tokens) are always loaded;
  this task activates one `SKILL.md` (~2,500 tokens). Scripts are executed, not read.
