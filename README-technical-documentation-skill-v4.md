...
│   │   │             check_freshness.py  check_posture.py
...
The plugin holds **664 tests** — unit, integration, conformance, security posture and
...
| Checked inside the archive too | `scripts/check_posture.py` ships with the skill: it reads every script on its syntax tree and exits 1 on a network import, unsafe loading, a shell, a command other than git, or a git subcommand that writes | `test_the_posture_check_holds_on_the_skill_and_catches_each_breach` |
