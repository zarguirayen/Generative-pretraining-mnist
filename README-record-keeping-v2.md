...
**Archive:** a GitHub Copilot plugin. The skill for this track is `aida-record-keeping`; it
uses `aida-core` for the scan, and the records it produces feed the two monitoring skills
and section 3.7 of the AIDA technical documentation. Python standard library only.
...
archive: section 3.7 is the record-keeping section this skill writes.
...
├── skills/
│   ├── aida-record-keeping/            ← this submission
...
│   ├── aida-operational-monitoring/    reads these records to compute Table 5
│   ├── aida-genai-monitoring/          reads the inference and ground-truth records
│   └── aida-human-oversight/           writes its interventions into the same chain
├── examples/ticket_agent/              a generated AIDA document to open directly
└── tests/  test_record_keeping.py  test_records_automation.py  test_runtime_integration.py …
```
