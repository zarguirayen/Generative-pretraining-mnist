...
2. **Route and answer** — maps each field of the official template to where its answer
   ...
   4 changes since the last documented state).
...
- **§4 Updates and changes, read from the git history** — the diff between the code and
  the last documented state (the state recorded when the document was written, else the
  last version tag) is classified: models added or removed, tools added, removed or
  re-parameterised, dependency, prompt, endpoint and deployment files changed, and every
  file this document cites that moved. A new tool, model or endpoint changes what the
  system can do, so it is named as a trigger for the change-of-usage procedure, whose
  outcome stays an open point. Each run records the documented state, so the next one
  compares against it (`DOC-UPDATE-ON-CHANGE`).
...
│   │   │             detect_changes.py  check_freshness.py
...
The repository holds **623 tests** — unit, integration, conformance, security posture,
